...
GO
   ...
   --
   -- Description :
   --              Load all sql server 2000 info from local server to snapshots table
   -- 	           

create procedure [dbo].[sp_sqlsecure_loadsql2000] (
@status nchar(1),
...
as
	...
	begin   
		...
		exec('INSERT INTO databaseobject (snapshotid, class, parentobjectid, dbid, objectid, name, type, owner) (select distinct ' + @snapshotid + ', -1, -1, dbid=' + @dbid + ', objectid=id, name, type=xtype, owner=uid from ' + @dbname + '.dbo.sysobjects)')
		...
		--set @tmpstr = 'INSERT INTO databasepermission (snapshotid, class, parentobjectid, dbid, uid, objectid, grantee, grantor, isgrantwith, isgrant, isrevoke, permission) (select distinct ' + @snapshotid + ', -1, -1, dbid=' + @dbid + ', uid=a.uid, objectid=a.id, b.grantee, b.grantor, isgrantwith=CASE WHEN a.protecttype = 204 THEN ''Y'' ELSE ''N'' END, isgrant=CASE WHEN a.protecttype = 205 THEN ''Y'' ELSE ''N'' END, isrevoke=CASE WHEN a.protecttype = 206 THEN ''Y'' ELSE ''N'' END, permission=CASE WHEN a.action = 26 THEN ''REFERENCES'' WHEN a.action = 178 THEN ''CREATE FUNCTION'' WHEN a.action = 193 THEN ''SELECT'' WHEN a.action = 195 THEN ''INSERT'' WHEN a.action = 196 THEN ''DELETE'' WHEN a.action = 197 THEN ''UPDATE'' WHEN a.action = 198 THEN ''CREATE TABLE'' WHEN a.action = 203 THEN ''CREATE DATABASE'' WHEN a.action = 207 THEN ''CREATE VIEW'' WHEN a.action = 222 THEN ''CREATE PROCEDURE'' WHEN a.action = 224 THEN ''EXECUTE'' WHEN a.action = 228 THEN ''BACKUP DATABASE'' WHEN a.action = 233 THEN ''CREATE DEFAULT'' WHEN a.action = 235 THEN ''BACKUP LOG'' WHEN a.action = 236 THEN ''CREATE RULE'' ELSE ''NONE'' END from ' + @dbname + '.dbo.sysprotects a, ' + @dbname + '.dbo.syspermissions b where a.id = b.id )'
		...
		exec('create view vw_sqlsecure_sysprotect (id, uid, columns, action, protecttype) as select id, uid, columns, action, protecttype from ' + @dbname + '.dbo.sysprotects')
		exec('create view vw_sqlsecure_syscolumns (name, id, colorder) as select name, id, colorder from ' + @dbname + '.dbo.syscolumns')
		exec('create view vw_sqlsecure_syspermissions (id, grantee, grantor) as select id, grantee, grantor from ' + @dbname + '.dbo.syspermissions')
