...
//
// 4/26/2004 - Trent Miskelly - Extended version task to include YearMonthDay BuildType,
//                              which simply uses DateTime.Now.ToString("yMMdd") to 
//								generate the version build number.
//								NOTE: I would have just extended the existing class, 
//								but some of the methods I need to override were private.
//
...
namespace Idera.SQLcompliance.Utility.Build
{
    /// <summary>
    ...
    public class VersionTaskEx : Task {
		...
		private int CalculateYearMonthDayBuildNumber() {
			return Convert.ToInt32(DateTime.Now.ToString("yMMdd"));
		}
        ...
        private int CalculateBuildNumber(int currentBuildNumber) {
            switch (BuildType) {
				case BuildNumberAlgorithm.YearMonthDay:
					return CalculateYearMonthDayBuildNumber();
        ...
        private Version CalculateVersion() {
			...
			if (newRevisionNumber == -1)
				...
				return new Version(version.Major, version.Minor, newBuildNumber, newRevisionNumber);
        }
            ...
            switch (RevisionType) {
                ...
                case RevisionNumberAlgorithm.Increment:
                    if (newBuildNumber != version.Build) {
                        // reset revision number to -1 (undefined) if the build number
                        // has changed
                        newRevsionNumber = -1;
                    } else {
                        // increment the revision number if this is a revision
                        // of the same build
                        newRevsionNumber = version.Revision == -1 ? 1 : version.Revision + 1;
                    }
                    break;
                case RevisionNumberAlgorithm.NoIncrement:
                    newRevsionNumber = version.Revision;
                    break;
                default:
