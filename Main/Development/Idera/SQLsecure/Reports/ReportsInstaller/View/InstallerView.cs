	...
	public class InstallerView : Idera.Common.ReportsInstaller.View.Wizard.WizardForm, IInstallerGUI
	{
		...
		public string SqlServerName
		{
			...
			{ 
                ...
                if (_sqlServerName.ToUpper().StartsWith("(LOCAL)"))
                {
                    server = _sqlServerName.ToUpper().Replace("(LOCAL)", Environment.MachineName);
                }
                else if ( _sqlServerName.StartsWith(".") )
                {
                    server = _sqlServerName.Replace(".", Environment.MachineName);
                }
                else if ( _sqlServerName == "localhost" )
                {
                    server = Environment.MachineName;
                }
                else if (_sqlServerName.StartsWith("localhost\\"))
                {
                    server = _sqlServerName.Replace("localhost", Environment.MachineName);
                }                    
				else
				{
					server = _sqlServerName;
				}
                return server;
			}
		}
		...
		public void PanelReportingServices_Next()
		{
            ...
            try
            {
                ...
                else
                {
                    ...
                    if ((checkBoxReportingServicesAdvancedSsl.Checked &&
                         textBoxReportingServicesAdvancedPort.Text.Equals("443"))
                        ||
                        (!checkBoxReportingServicesAdvancedSsl.Checked &&
                         textBoxReportingServicesAdvancedPort.Text.Equals("80")))
                    {
                    ...
                    {
                        if (
                            modelAdapter.TestReportManager(_computerPortName,
                                                           textBoxReportingServicesAdvancedManager.Text))
                        {
                            if (
                                modelAdapter.TestReportServer(_computerPortName,
                                                              textBoxReportingServicesAdvancedDirectory.Text))
                            {
                                if (
                                    modelAdapter.HasAppropriatePermissions(_computerPortName,
                                                                           textBoxReportingServicesAdvancedDirectory.
                                                                               Text))
                                {
                                    if (_radioInstallChecked)
		    ...
		    {
		        DisplayMessageBox(string.Format("Error:  {0}", ex.Message), "Reporting Service Installer Error");
                PanelReportingServices_Helper(true);
		    }
		}
            ...
            try
            {
                ...
                else
                {
                    ...
                    else if (modelAdapter.TestConnection(SqlServerName) == false)
                    {
                        ...
                        modelAdapter.ConfirmPassword(textBoxRepositoryInstallLogin.Text,
                                                     textBoxRepositoryInstallPassword.Text) == false)
                    {
                    }
                    else if (
                        !modelAdapter.ValidateFolderName(_computerPortName,
                                                         textBoxReportingServicesAdvancedDirectory.Text,
                                                         textBoxRepositoryInstallFolder.Text))
                    {
                    }
                    ...
                    {
                        if (
                            modelAdapter.DoesFolderExist(_computerPortName,
                                                         textBoxReportingServicesAdvancedDirectory.Text,
                                                         textBoxRepositoryInstallFolder.Text))
                        {
                            _createNewFolder = false;
                            if (
                                modelAdapter.DoesDataSourceExist(_computerPortName,
                                                                 textBoxReportingServicesAdvancedDirectory.Text,
                                                                 textBoxRepositoryInstallFolder.Text))
                            {
                        ...
                        {
                            bool contin =
                                DisplayMessageBoxYesNo("The folder <" + textBoxRepositoryInstallFolder.Text +
                                                       "> does not exist.  The installer can create a new folder with that name and path for you.  Is this okay?",
                                                       "Create New Folder?");
                            if (contin)
                            {
                                _createNewFolder = true;
            ...
            try
            {
                ...
                else
                {
                    ...
                    else if (modelAdapter.TestConnection(SqlServerName) == false)
                    {
                        ...
                        modelAdapter.ConfirmPassword(textBoxRepositoryChangeLogin.Text,
                                                     textBoxRepositoryChangePassword.Text) == false)
                    {
                        PanelRepositoryChange_Helper(true);
                    }
                    else if (
                                                      ...
                                                      textBoxRepositoryChangeFolder.Text))
                    {
                        DisplayMessageBox(
                            textBoxRepositoryChangeFolder.Text +
                            " does not exist.  Please specify a folder that already exists.",
                            "Invalid Folder Name!");
                        PanelRepositoryChange_Helper(true);
                    }
                    else
