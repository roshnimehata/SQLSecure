...
//                _rdlFolderPath = Path.Combine(System.Windows.Forms.Application.StartupPath, _rdlFolder);
			...
			string folderPath)
		{
			...
			foreach (CatalogItem ci in items)
			{
				...
				{
                    if (_dictReportFile.ContainsKey(ci.Name))
                    {
...
//				stream.Close();
			...
			string reportManager)
		{
			...
			{
				_viewAdapter.UpdateInstallerLog("The setup program is unable to insert the Reporting Services Instance into the SQLsecure database.\r\n  Please verify that you have insert rights on this database.\r\n");
				return false;
			}
		}
		...
		public bool DoesReportsXMLFileExist()
		{
			...
			if (check == false)
			{
                _viewAdapter.DisplayMessageBox(Path.Combine(_rdlFolderPath, _xmlReportsName) + " file not found", "Error!");
			}
			return check;
		}
                ...
                using (DataTable servers = serversEnum.GetDataSources())
                {
                    ...
                    foreach (DataRow row in servers.Rows)
                    {
                        ...
                        if (!row.IsNull("ServerName"))
                        {
                            ...
                            _viewAdapter.AddItemToSQLInstanceList(serverInstance.ToUpper());
                        }
                    }
                }
            }
