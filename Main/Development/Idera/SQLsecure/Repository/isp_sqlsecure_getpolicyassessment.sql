    ...
    IF ( @assessmentid IS NULL
        ...
        BEGIN
            ...
            EXEC [dbo].[isp_sqlsecure_createassessmentfrompolicy] @policyid = @policyid,
                @assessmentid = NULL, @type = N'C', @copy = 2,
    ...
    IF ( @isadmin = 1 )
		...
		-- check to make sure the assessment data is current.
            EXEC [dbo].[isp_sqlsecure_isassessmentdatacurrent] @policyid = @policyid,
                @assessmentid = @assessmentid, @valid = @valid OUTPUT;
        END;
    ...
    ELSE
		...
		-- if it is a current assessment, then refresh the metric settings
                    ...
                    IF ( @isadmin = 1 )
                        BEGIN
                            IF ( @debug = 1 )
                                ...
                                END;
                            EXEC [dbo].[isp_sqlsecure_createassessmentfrompolicy] @policyid = @policyid,
                                @assessmentid = @assessmentid, @type = N'C',
                                @copy = 3,
                                @newassessmentid = @assessmentid OUTPUT;
                        END;
                END;
			...
			-- get the list of metrics for the policy
                    ...
                    DECLARE metriccursor CURSOR STATIC
                    FOR
                        SELECT  metricid ,
                                metricname ,
                                metrictype ,
			...
			-- process the snapshots for each metric
                    ...
                    DECLARE snapcursor CURSOR STATIC
                    FOR
                        SELECT  a.snapshotid ,
				...
				-- save a list of sysadmin members in this snapshot for use by multiple metrics
						...
						-- Collection Time
							...
							-- remove the quotes surrounding the value and test for numeric
                                                ...
                                                SELECT  @metricthreshold = N'Audit data is acceptable if within '
                                                        + @severityvalues
                            ...
                            --find the matching entry based on major and minor version
								...
								--parse the server version into component numbers to compare string values of different lengths
                                                            ...
                                                            IF ( @intval > 1 )
                                                              ...
                                                              SELECT
                                                              @v1 = CASE
                                                              WHEN ISNUMERIC(@ver) = 1
                                                              THEN CONVERT(INT, @ver)
                                                              ELSE 0
                                                              END ,
                                                              @strval = RIGHT(@strval,
                                                              LEN(@strval)
                                                              ...
                                                              SELECT
                                                              @v3 = CASE
                                                              WHEN ISNUMERIC(@strval) = 1
                                                              AND CHARINDEX('.',
                                                              @strval) = 0
                                                              THEN CONVERT(INT, @strval)
                                                              ELSE 0
                                                              END;
                                                              END;
                                                              ELSE
								...
								--compare level by level to see if there is a mismatch
                                                    ...
                                                    SELECT  @metricval = N'Current version is '
                                                            + @version;
                                                    SELECT  @metricthreshold = N'Acceptable levels for each SQL Server version are '
                                                            + @severityvalues
                                                            + N' and above.';
                                                END;
						...
						-- Guest User Enabled
												...
												select databasename
													...
													order by databasename';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Database with Guest user access: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'DB' , -- object type
						...
						-- Suspect Logins
							...
							-- This should return the same results as the report SP
                                                              ...
                                                              FROM
                                                              serverprincipal a
                                                              INNER JOIN windowsaccount b ON a.snapshotid = b.snapshotid
                                                              AND a.sid = b.sid
                                                              LEFT JOIN ancillarywindowsgroup c ON a.snapshotid = c.snapshotid
						...
						-- Integration Services
													...
													select a.name as objectname, c.name as granteename, b.permission
														...
														order by objectname, granteename';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Permissions on stored procedure found: '
                                                              + @strval3
                                                              + N' granted to '''
                                                              + @strval2
                                                              + N''' on '''
														...
														where snapshotid = '
															...
															and [type] in (N''P'', N''X'')
															 and runatstartup = N''Y''';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Unapproved stored procedures found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'P' , -- object type
                                                              ...
                                                              + CASE
                                                              WHEN LEN(@severityvalues) > 0
                                                              THEN N' and are not in '
                                                              + @severityvalues
                                                              + N'.'
                                                              ELSE N'.'
                                                              END;
						...
						-- Stored Procedures Encrypted
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Unencrypted stored procedures found in the database: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'DB' , -- object type
													...
													select [name]
														...
														where snapshotid = '
															...
															and userdefined = N''Y''';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Unapproved extended stored procedure found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'X' , -- object type
						...
						-- Dangerous Extended Stored Procedures (XPs)
													...
													select a.name as objectname, c.name as granteename, b.permission
														...
														order by objectname, granteename';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Permission on extended stored procedures found: '''
                                                              + @strval3
                                                              + N' granted to '''
                                                              + @strval2
                                                              + N''' on '''
						...
						-- Common TCP Port Used
							...
							--	dynamic must now be entered in the list to be a finding
															--	or dynamicport = N''Y'')
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Unapproved TCP/IP port found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
						...
						-- Unexpected Registry Key Owners
													...
													select objectname, ownername
														...
														where snapshotid = '
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Registry key with unapproved owners found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
						...
						-- Unexpected Registry Key Permissions
													...
													select objectname, grantee
														...
														where snapshotid = '
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Registry key with unapproved permissions found: '''
                                                              + @strval
                                                              + N''' has grantee '''
                                                              + @strval2
                                                              + N'''' ,
						...
						-- Unexpected Executable File Owners
													...
													select distinct lower(objectname) as objectname, ownername
														...
														where snapshotid = '
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'File with unapproved owner found: '''
                                                              + @strval
                                                              + N''' has owner '''
                                                              + @strval2
                                                              + N'''' ,
						...
						-- Unexpected Executable File Permissions
													...
													select distinct lower(objectname) as objectname, grantee
														...
														where snapshotid = '
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'File with unapproved permission found: '''
                                                              + @strval
                                                              + N''' has grantee '''
                                                              + @strval2
                                                              + N'''' ,
						...
						-- Everyone Database File Access
								...
								--check for permissions
                                                              ...
                                                              N'P' AS [type]
                                                              FROM
                                                              vwfilesystemobjectpermission a
                                                              WHERE
                                                              a.snapshotid = @snapshotid
									...
									--check for file owners of no permissions found
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'File with Everyone access found: '''
                                                              + @strval
                                                              + N''' has '
                                                              + CASE
                                                              WHEN @type = 'O'
						...
						-- Operating System Version
							...
							-- they will be trimmed from both the os version and the match list for consistency
                                                              ...
                                                              INTO @tblval
                                                              VALUES
                                                              ( LTRIM(RTRIM(SUBSTRING(@severityvalues,
                                                              @intval + 1,
                                                              @intval2
                                                              - @intval - 1))) );
                                                              SELECT
                                                              ...
                                                              + CASE
                                                              WHEN @strval IS NULL
                                                              THEN N'unknown. Check the snapshot status and the activity log for possible causes.'
                                                              ELSE @os + '.'
                                                              END;
						...
						-- Complete Audits
							...
							-- Check that filters do not exclude anything
                                                              ...
                                                              matchstring
                                                              FROM
                                                              SQLsecure.dbo.vwsnapshotfilterrules
                                                              WHERE
                                                              snapshotid = @snapshotid
                                                              ORDER BY rulename ,
                                                              classname;
							...
							-- if everything else is ok, then check and make sure all objects were included
                                                              ...
                                                              FROM
                                                              filterruleclass a
                                                              LEFT JOIN vwsnapshotfilterrules b ON ( a.objectclass = b.class
                                                              AND b.snapshotid = @snapshotid
                                                              )
                                                              WHERE
                                                              b.snapshotid IS NULL;
                                                              IF ( @version > N'9.'
                                                              OR @version < N'6.'
                                                              )
                                                              BEGIN
						...
						-- Password Policy Enabled
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Password policy not enforced on '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iLOGN' , -- object type
						...
						-- 'Blank Passwords
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Login with blank password found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iLOGN' , -- object type
						...
						-- 'Fixed roles assigned to public or guest
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Public or guest is a member of a fixed role on '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'DB' , -- object type
		...
		--************************************************* version 2.5 security checks
						-- Password Expiration Enabled
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Password expiration not enabled for '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iLOGN' , -- object type
		...
		--************************************************* version 2.6 security checks
						...
						-- Unauthorized Accounts are sysadmins
													...
													select distinct name 
														...
														where lower(name) not in ('
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Unauthorized sysadmin member found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iLOGN' , -- object type
							...
							-- only apply this check if the version is 2005 or greater
														...
														select b.name 
															...
															where a.snapshotid='
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'ALTER TRACE permission granted to '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iSRV' , -- object type
							...
							-- only apply this check if the version is 2005 or greater
														...
														select b.name 
															...
															where a.snapshotid='
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'CONTROL SERVER permission granted to '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iSRV' , -- object type
						...
						-- Required Admin Accounts
								...
								-- convert the severityvalues into a table for reverse selection
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Required administrative login not found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iSRV' , -- object type
						...
						-- Database files missing required administrative permissions
                                                              ...
                                                              1
                                                              FROM
                                                              vwfilesystemobjectpermission p
                                                              WHERE
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Database file missing required permission found: '''
                                                              + @strval
                                                              + N''' is missing Full Control for '''
                                                              + @strval2
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'FI' , -- no file object type
						...
						-- Executable files missing required administrative permissions
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Executable file missing required permission found: '''
                                                              + @strval
                                                              + N''' is missing Full Control for '''
                                                              + @strval2
                                                              + N'''' ,
						...
						-- Registry Keys missing required administrative permissions
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Registry key missing required administrative permission found: '''
                                                              + @strval
                                                              + N''' is missing Full Control for '''
                                                              + @strval2
                                                              + N'''' ,
						...
						-- Data Files on System Drive
							...
							-- It will just produce findings on any data file
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Data file found on system drive: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'FI' , -- no file object type
						...
						-- SQL Server Installation on System Drive
								...
								-- It will just produce findings on any data file
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Installation directory found on system drive: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
						...
						-- Unauthorized SQL Logins exist
							...
							-- It will just produce findings on anything although I guess there will always be an sa account and a finding.
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Unauthorized SQL Login found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iLOGN' , -- object type
						...
						-- Public Server Role Has Permissions
							...
							-- only apply this check if the version is 2005 or greater
											...
											-- note this matches the functionality of isp_sqlsecure_getserverprincipalpermission for returning the object name
                                                              CASE
                                                              WHEN classid = 101
                                                              THEN dbo.getclasstype(a.classid)
                                                              + N' '''
                                                              + ( SELECT
                                                              name
                                                              FROM
						...
						-- Databases Are Trustworthy
												...
												select databasename
													...
													order by databasename';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Database with Trustworthy bit enabled: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'DB' , -- object type
						...
						-- Sysadmins Own Trustworthy Databases
												...
												select databasename
													...
													order by databasename';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Trustworthy Database owned by sysadmin: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'DB' , -- object type
						...
						-- Public Roles Have Permissions on User Databases
                                                              ...
                                                              FROM
                                                              vwdatabaseobjectpermission a
                                                              INNER JOIN sqldatabase b ON ( ( a.dbid = b.dbid )
                                                              AND ( a.snapshotid = b.snapshotid )
                                                              )
                                                              INNER JOIN databaseschema c ON ( ( a.schemaid = c.schemaid )
                                                              AND ( a.dbid = c.dbid )
                                                              AND ( a.snapshotid = c.snapshotid )
                                                              )
                                                              WHERE
						...
						-- Dangerous Security Principals
								...
								order by dpgroup.name';
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Dangerous security principals found in SSIS database roles: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iDUSR' , -- object type
						...
						-- Integration Services Roles Permissions Not Acceptable
											...
											(
												...
												and (dp.name not in (''db_dtsadmin'', ''db_dtsltduser'', ''db_dtsoperator'', ''db_ssisadmin'', ''db_ssisltduser'', ''db_ssisoperator''))
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Permissions on stored procedures found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'DB' , -- object type
						...
						--Weak Passwords
                                                              ...
                                                              @metricid ,
                                                              @snapshotid ,
                                                              N'Login with weak password found: '''
                                                              + @strval
                                                              + N'''' ,
                                                              NULL , -- database ID,
                                                              N'iLOGN' , -- object type
                                        ...
                                        IF ( @metricid = 93 )
                                            BEGIN 
                                                ...
                                                IF ( @isadmin = 1 )
                                                    BEGIN 
                                                        INSERT
                                                              ...
                                                              JOIN dbo.sqldatabase sd ON db.snapshotid = sd.snapshotid
                                                              AND db.dbid = sd.dbid
                                                              WHERE
                                                              db.type = 'isk'
                                                ...
                                                SELECT  @metricthreshold = N'Server is vulnerable if system database have symmetric key '; 
                                            END; 
                                        ...
                                        ELSE
                                            IF ( @metricid = 94 )
                                                BEGIN 
                                                    ...
                                                    SELECT  @metricthreshold = N'Server is vulnerable if there are user defined assemblies with host policy other than SAFE ';
                                                END; 
							   ...
							   ;
                                                              WITH
                                                              users ( objectid, name, isOwner, snapshotid, userid, dbid, type )
                                                              AS ( SELECT
                                                              db.objectid ,
                                                              db.name ,
                                                              0 AS IsOwner ,
                                                              ...
                                                              FROM
                                                              dbo.databaseobject db
                                                              JOIN dbo.databaseobjectpermission dp ON db.snapshotid = dp.snapshotid
                                                              AND db.dbid = dp.dbid
                                                              ...
                                                              AS rm ON m.snapshotid = rm.snapshotid
                                                              AND m.dbid = rm.dbid
                                                              AND m.uid = rm.rolememberuid
                                                              JOIN databaseprincipal
                                                              AS r ON rm.snapshotid = r.snapshotid
                                                              AND rm.dbid = r.dbid
                                                              AND rm.groupuid = r.uid
                                                              JOIN dbo.serverprincipal sm ON m.snapshotid = sm.snapshotid
                                                              AND m.usersid = sm.sid
                                                              JOIN dbo.serverrolemember srm ON sm.snapshotid = srm.snapshotid
                                                              AND sm.principalid = srm.memberprincipalid
                                                              JOIN dbo.serverprincipal sr ON rm.snapshotid = sr.snapshotid
                                                              ...
                                                              CASE u.isOwner
                                                              WHEN 1
                                                              THEN N'Startup stored procedure '
                                                              + u.name
                                                    ...
                                                    ELSE
                                                        IF ( @metricid = 97 )
                                                            BEGIN 
                                                              ...
                                                              JOIN dbo.serveroswindowsgroupmember ss ON swa.snapshotid = ss.snapshotid
                                                              AND swa.sid = ss.groupmember
                                                              JOIN dbo.serveroswindowsaccount smm ON ss.snapshotid = smm.snapshotid
                                                              ...
                                                              FROM
                                                              dbo.sqljobproxy p
                                                              JOIN dbo.serveroswindowsaccount swa ON p.snapshotid = swa.snapshotid
    				    ...
    				    --  DISTRIBUTOR_ADMIN account
                                                        ELSE
                                                            IF ( @metricid = 98 )
                                                              ...
                                                              SPU.name
                                                              FROM
                                                              dbo.serverrolemember
                                                              AS SRM
                                                              INNER JOIN dbo.serverprincipal
                                                              AS SPU ON SRM.snapshotid = SPU.snapshotid
                                                              AND SRM.memberprincipalid = SPU.principalid
                                                              AND SPU.type <> 'R'
                                                              INNER JOIN dbo.serverprincipal
                                                              AS SPR ON SRM.snapshotid = SPR.snapshotid
  						...
  						-- sysadmin accounts with local administrator role
                                                            ELSE
                                                              ...
                                                              AS WGM ON SPU.snapshotid = WGM.snapshotid
                                                              AND SPU.sid = WGM.groupmember
                                                              INNER JOIN dbo.serveroswindowsaccount
                                                              AS WG ON WGM.snapshotid = WG.snapshotid
 						...
 						--information about database roles
                                                              ...
                                                              rolepermissions
                                                              FROM
                                                              dbo.databaseprincipal
                                                              AS DPR
                                                              INNER JOIN dbo.sqldatabase
                                                              AS DB ON DPR.snapshotid = DB.snapshotid
                                                              AND DPR.dbid = DB.dbid
                                                              AND DPR.type = 'R'
                                                              LEFT JOIN @DatabaseRoleUsers
                                                              AS DRU ON DPR.snapshotid = DRU.snapshotid
                                                              ...
                                                              DECLARE DatabaseRolesInfoCursor
                                                              CURSOR
                                                              FOR
                                                              SELECT
                                                              DRI.databasename ,
...
--						--information about server roles
						...
						--*
                                                              ...
                                                              AS ST ON SP.snapshotid = ST.snapshotid
                                                              AND SP.type = 'R'
                                                              LEFT JOIN @ServerRoleUsers
                                                              AS SRU ON SP.snapshotid = SRU.snapshotid
                                                              ...
                                                              *
                                                              ...
                                                              JOIN dbo.sqldatabase sdb ON dd.snapshotid = sdb.snapshotid
                                                              AND dd.dbid = sdb.dbid
                                                              JOIN dbo.databaseschema ds ON dd.snapshotid = ds.snapshotid
                                                              AND dd.dbid = ds.dbid
                                                              AND dd.schemaid = ds.schemaid
                                                              JOIN dbo.databaseobjectpermission dp ON dd.snapshotid = dp.snapshotid
                                                              AND dd.dbid = dp.dbid
                                                              ...
                                                              *
                                                              ...
                                                              WITH
                                                              OrphanedUsers ( dbid, name, uid, type )
                                                              AS ( SELECT
                                                              dbid ,
                                                              name ,
                                                              uid ,
                                                              ...
                                                              *
                                                              ...
                                                              OrphanedUsers;
                                                              WITH
                                                              OrphanedUsersForDb ( orphanedUsersCountForDb, orphanedUsersnamesForDb, dbname, dbid )
                                                              AS ( SELECT
                                                              ...
                                                              #tempdetails c2
                                                              WHERE
                                                              c.databaseid = c2.databaseid
                                                              FOR
                                                              XML
                                                              PATH(N'')
                                                              ), 1, 2, N'') AS orphanedUsersnamesForDb ,
                                                              sdb.databasename ,
                                                              c.databaseid
                                                              FROM
                                                              #tempdetails c
                                                              JOIN dbo.sqldatabase sdb ON c.databaseid = sdb.dbid
				...
				-- Other General Domain Accounts Check
							...
							;
                                                              WITH
                                                              ConvertedSidsForCurrentSnapshot ( [sid], name )
                                                              AS ( SELECT
                                                              ...
                                                              INSERT
                                                              INTO @generalUserPrincipals
                                                              SELECT
                                                              [name]
                                                              FROM
                                                              ConvertedSidsForCurrentSnapshot
                                                              WHERE
                                                              [sid] LIKE @domainUsersSidPattern
                                                              OR [sid] IN (
                                                              @everyoneSidString,
				...
				-- SQL Jobs and Agent Check
					    ...
					    ---Encryption methods 
                                                              ...
                                                              FROM
                                                              dbo.encryptionkey e
                                                              JOIN dbo.sqldatabase s ON s.dbid = e.databaseid
                                                              AND e.snapshotid = s.snapshotid
                                                              WHERE
                                                              e.snapshotid = @snapshotid
                                                              AND e.algorithmdesc NOT IN (
                                                              SELECT
                                                              Value
                                                              FROM
                                                              dbo.splitbydelimiter(@severityvalues,
					...
					--****************************** done processing the security check. Now write out the valid metric data ***********************************
						...
						-- handle unexpected null values more gracefully
                                            SELECT  @metricval = ISNULL(@metricval,
...
--			end
			...
			-- if any servers are left in the server table, there was no audit data and this is a finding
				...
				-- Audited Servers
                            ...
                            IF ( @metricid = 54 )	--	only process if the metric was enabled
                                BEGIN
                                    DECLARE servercursor CURSOR STATIC
                                    FOR
                                        SELECT  registeredserverid ,
                                                connectionname
                                        FROM    registeredserver
                                        WHERE   registeredserverid IN (
                                                SELECT  registeredserverid
                                                FROM    #servertbl );
                                    OPEN servercursor;
			...
			-- log the changes
                    ...
                    EXEC isp_sqlsecure_addpolicychangelog @policyid = @policyid,
                        @assessmentid = @assessmentid,
                        @state = @assessmentstate, @description = @msg;
                END;
    ...
    SELECT  a.* ,
            ...
            + CASE WHEN severitycode > 0
                        AND ISNULL(b.isexplained, 0) = 0 THEN 10
                   ELSE 0
              END ,
            notes = ISNULL(b.notes, N'')
    FROM    @outtbl a
            LEFT JOIN policyassessmentnotes b ON b.policyid = @policyid
